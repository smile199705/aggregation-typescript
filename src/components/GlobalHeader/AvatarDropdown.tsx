...
const AvatarDropdown: React.FC = () => {
  ...
  // return loginUser ? (<Dropdown overlayClassName={classNames(styles.container)} overlay={menuHeaderDropdown}>
  // @ts-ignore
  return loginUser ? (<Dropdown overlay={ menuHeaderDropdown }>
      <div className={`${styles.action} ${styles.account}`}>
        {/*<Avatar>{loginUser.userName?.[0] ?? '无'}</Avatar>*/}
        <Avatar style={{ backgroundColor: '#87d068' }} icon={<UserOutlined />} >{loginUser.userName?.[0] ?? '无'}</Avatar>
      </div>
    </Dropdown>
  ) : (
    <>
